...
use fuser::{
    ...
    ReplyWrite, ReplyCreate, ReplyEmpty, Request, // <--- Added ReplyEmpty
};
...
impl Filesystem for BetterFS {
    ...
    fn lookup(&mut self, _req: &Request, parent: u64, name: &OsStr, reply: ReplyEntry) {
        ...
        // Check Backend OR our write buffer (if we just created it but didn't save yet)
        let inode = calculate_inode(filename);
        
        let size = if let Some(buffer) = self.open_files.get(&inode) {
            buffer.data.len() as u64
        } else if let Some(s) = self.manager.get_file_metadata(filename) {
            s
        } else {
        ...
        let attr = FileAttr {
            ...
            perm: 0o644, nlink: 1, uid: 1000, gid: 1000, rdev: 0, flags: 0, blksize: 512,
        };
    ...
    fn readdir(&mut self, _req: &Request, ino: u64, _fh: u64, offset: i64, mut reply: ReplyDirectory) {
        if ino != 1 { return reply.error(ENOENT); }
        ...
        for filename in self.manager.list_files() {
             entries.push((calculate_inode(&filename), FileType::RegularFile, filename));
        }
        
        // Add files currently being written (in buffer)
        for buffer in self.open_files.values() {
            entries.push((calculate_inode(&buffer.filename), FileType::RegularFile, buffer.filename.clone()));
        }

        for (i, entry) in entries.into_iter().enumerate().skip(offset as usize) {
            if reply.add(entry.0, offset + i as i64 + 1, entry.1, entry.2) { break; }
        }
        reply.ok();
    }
    ...
    fn create(&mut self, _req: &Request, parent: u64, name: &OsStr, _mode: u32, _umask: u32, _flags: i32, reply: ReplyCreate) {
        if parent != 1 { return reply.error(ENOENT); }
        ...
        let attr = FileAttr {
            ino: inode, size: 0, blocks: 1,
            atime: UNIX_EPOCH, mtime: UNIX_EPOCH, ctime: UNIX_EPOCH, crtime: UNIX_EPOCH,
            kind: FileType::RegularFile, perm: 0o644, nlink: 1, uid: 1000, gid: 1000, rdev: 0, flags: 0, blksize: 512,
        };
